...
def float_list_vertex(listt): #zwraca vertex jako float
    ...
    for i in range(len(listt)):
        ...
        tmp[i]=['v',list_t]
    return tmp
def xy_minmax(xy_list, sorted_block_size): # wznaczenie x y (min i max)    
   # xy_list = np.empty((len(l_face), 4)) ############################################
    block_size = np.empty((len(l_face), 2))
    last = len(area) - 1 # ostatni wektor z area
    for face in range((len(l_face))):
        ...
        x_temp = [xa, xb, xc]
        y_temp = [ya, yb, yc]
        x_min = min(x_temp) -3 # redukcja krawedzi łaczacych trojkaty
        x_max = max(x_temp) +3
        y_min = min(y_temp) -3
        y_max = max(y_temp) +3
        ...
        if (x_min<0 or y_min<0 or x_max>=width or y_max>=height or xa<0 or xa>=width or xb<0 or xb>=width or xc<0 or xc>=width or ya<0 or ya>=height or yb<0 or yb>=height or yc<0 or yc>=height):
            ...
            val = 0
            position = 0
            for pic in range(len(list_of_images)): # usówanie czarnych trójkątów wewnątrz modelu
                if area[pic][face] > val and p != pic:
                    val = area[pic][face]
                    position = pic 
...
pos = np.empty((len(list_of_images),len(l_vert),2)) # pos [picture][point][x or y]
for pic in range(len(list_of_images)): 
    P = K.dot(R_t[pic])
    for pnt in range(len(l_vert)):
        M=[l_vert[pnt][1][0],l_vert[pnt][1][1],l_vert[pnt][1][2],1]
        zm = P.dot(M)
        pos[pic][pnt][0] = round(zm[0]/zm[2]) #składowa x
        pos[pic][pnt][1] = round(zm[1]/zm[2]) #składowa y
print(pos[0][0][0])
print(pos[0][0][1])
area = np.empty((len(list_of_images) + 1,len(l_face)))
area2 = [[] for y in range((len(l_face)))]

 # wyznaczanie wielkości trójkątów w celu wyboru zdj o największym trójkącie

for face in range((len(l_face))):  
    a = l_face[face][1][0]
    b = l_face[face][1][1]
    c = l_face[face][1][2]
    for pic in range(len(list_of_images)): #petla po zdj
        xa = pos[pic][a][0]
        ya = pos[pic][a][1]
        xb = pos[pic][b][0]
        yb = pos[pic][b][1]
...
# pszukiwanie najwiekszych trojkatow ze zbioru zdj
for face in range((len(l_face))): 
    for iter in range(len(list_of_images)):
        al = 0
        osition = 0
        or pic in range(len(list_of_images)):
           if area[pic][face] > val:
               val = area[pic][face]
               position = pic 
        rea2[face].append(position)
        rea[position][face] = -1
    area[pic + 1][face] = area2[face][0]

# wznaczenie x y (min i max) 
xy_list = np.empty((len(l_face), 4)) 
sorted_block_size = np.empty((len(l_face), 2))
[xy_list, sorted_block_size ] = xy_minmax(xy_list, sorted_block_size)

# redukcja rozbieżności trójkątów
size_factor = 0.1
i = 0
h=(len(l_vert))
last = len(area) - 1
...
# zapis bloków do pliku PNG
dimension = round(2 * height) 
texture = np.full((dimension, dimension, 3), 0)  #pamiec na texture  ############################
black_block = np.zeros((500,500,3))
i = len(sorted_block_size) - 1
UV_pos = np.empty((len(sorted_block_size), 6))
index_x = 0 # zakres w ktorym wypelniony jest PNG
index_y = 0
...
while i >= 0:
    ...
    if ((diff_y + index_y )>=dimension):
        ...
        texture_num += 1
        texture = np.full((dimension, dimension, 3),0)###########################################################
        index_x = 0 # zakres w ktorym wypelniony jest PNG
        index_y = 0
        index_xmax = dimension
        index_ymax = 0
...
for pnt in range(len(l_vert)): #petla po punktach
    obj_file.write("v  ")
