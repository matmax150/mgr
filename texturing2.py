...
for face in range((len(l_face))):
    ...
    x_temp = [xa, xb, xc]
    y_temp = [ya, yb, yc]
    xy_list[face][0] = min(x_temp)
    xy_list[face][1] = max(x_temp)
    xy_list[face][2] = min(y_temp)
    xy_list[face][3] = max(y_temp)
    block_size[face][0] = face
    if (xa<0 or xa>=width or xb<0 or xb>=width or xc<0 or xc>=width or ya<0 or ya>=height or yb<0 or yb>=height or yc<0 or yc>=height):
        block_size[face][1] = 0
    else:
        block_size[face][1] = (max(x_temp)-min(x_temp)) * (max(y_temp)-min(y_temp))
...
texture = np.empty((dimension, dimension, 3))  #pamiec na texture  
black_block = np.empty((500,500,3))
...
while i >= 0:
    ...
    if (index_x + diff_x + 1) >= index_xmax: ###############################################################
        index_x = 0
        index_y = index_ymax + 1 
    ...
    if ((diff_y + index_y )>=dimension):
        ...
        texture_num += 1
        texture = np.empty((dimension, dimension, 3))
        index_x = 0 # zakres w ktorym wypelniony jest PNG
        index_y = 0
        index_xmax = dimension
        index_ymax = 0
